...
(function (context) {
  ...
  function deepMerge_(obj1, obj2) {
    ...
    Object.entries(obj2).forEach(function (p) {
      if (p[0] in obj && _typeof(obj[p[0]]) === "object" && obj[p[0]] !== null && _typeof(p[1]) === "object") {
   ...
   */
  GeoChart.prototype.draw = function (data) {
    ...
    this.map_.data.loadGeoJson(this.options_.geoJson, this.options_.geoJsonOptions, function (features) {
      ...
      if (this.options_.legend !== "none" && this.data_has_values_ && this.data_.getNumberOfRows()) {
        ...
        control_position = google.maps.ControlPosition[this.options_.legend.position];
        this.map_.controls[control_position].push(this.legend_.getContainer());
      }
  ...
  var ColorAxis = function ColorAxis(geoChart) {
  ...
  ColorAxis.prototype.getRelativeColors = function (rel_pos) {
    ...
    rel_colors = [get_relative_color(this.colors_, rel_pos), get_relative_color(this.stroke_colors_, rel_pos)];
  ...
  var Tooltip = function Tooltip(geoChart) {
    this.geo_chart_ = geoChart;
  ...
  Tooltip.prototype.onAdd = function () {
    ...
    var p_style = Object.assign({}, { margin: TOOLTIP_MARGIN + "px" }, processTextStyle_(this.geo_chart_.options_.tooltip.textStyle));
  ...
  var Legend = function Legend(geoChart) {
    this.geo_chart_ = geoChart;
  ...
  Legend.prototype.draw_ = function () {
    ...
    Object.assign(div_inner.style, { marginTop: -LEGEND_INDICATOR_TOP_OFFSET + "px" }, processTextStyle_(this.geo_chart_.options_.legend.textStyle));

    var min_div = document.createElement("div");
    min_div.style.padding = LEGEND_NUM_PADDING_VERT + "px " + LEGEND_NUM_PADDING_HORIZ + "px";
    ...
    max_div.style.padding = LEGEND_NUM_PADDING_VERT + "px " + LEGEND_NUM_PADDING_HORIZ + "px";
